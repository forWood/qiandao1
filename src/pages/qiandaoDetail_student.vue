<template>
  <div class="home">
    ...
    <div>
      ...
      <!--</select>-->
      <span style="text-align: center;font-size: 12px;position: relative;left: 20px"> {{a}}正常 {{b}}迟到 {{c}}缺席 {{d}}早退 {{e}}请假 {{f}}未签到</span>
...
<script>
  ...
  export default {
    ...
    //  this.telphone=this.$route.query.telphone;
      //console.log(this.Class_No+''+ this.Course_Name+'');
      //显示所有该成员这门课的所有签到记录
      this.$ajax({
        method:'get',
        ...
        for(var i=0;i<response.data.length;i++){
          ...
          if(response.data[i].SignState==0){
            response.data[i].SignState='未签到';
            this.f++;
          }
        }
        //console.log(response.data);
